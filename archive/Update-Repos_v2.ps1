    ...
    if (Test-Path $gitPath) {
        ...
        if ($gitStatus) {
            ...
            foreach ($line in $gitStatus) {
                if ($line.Length -gt 3) {
                    # Extract the relative file path from git status (strip status codes and quotes)
                    ...
                    if ($isConfig) {
                        $hasConfigFiles = $true
                        $filesToDelete += Join-Path $dir.FullName $relPath
                    }
                }
            }
            ...
            # Action: Backup and delete target config files if found
            if ($hasConfigFiles) {
                ...
                foreach ($file in $filesToDelete) {
                    if (Test-Path $file) {
                        Write-Host "  Deleting configuration file: $(Split-Path $file -Leaf)" -ForegroundColor Gray
        ...
        # Try to checkout main (will execute if there were no changes, or if changes were successfully cleared)
        Write-Host "Processing Git repo: $($dir.Name)" -ForegroundColor Cyan
        git -C $dir.FullName checkout main 2>$null
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "Could not checkout 'main' branch in '$($dir.Name)' (Does it use 'master' or another default branch name?)."
            continue
        }
